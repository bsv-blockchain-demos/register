...
export default class VCTopicManager implements TopicManager {
  ...
  async identifyAdmissibleOutputs(beef: number[], previousCoins: number[]): Promise<AdmittanceInstructions> {
    ...
    try {
      console.log('VC topic manager was invoked')
      ...
      console.log('VC topic manager has parsed a the transaction: ', parsedTransaction.id('hex'))
      ...
      for (const [i, output] of parsedTransaction.outputs.entries()) {
        ...
        try {
          ...
          if (result.fields.length !== 2) {
            throw new Error('VC token must have exactly one field + signature')
          }

          const serialNumber = Utils.toUTF8(result.fields[0])

          if (serialNumber === undefined || serialNumber === null) {
            throw new Error('VC token must contain a valid serialNumber')
          }

          // Check that the field is exactly 32 bytes in length
          if (result.fields[0].length !== 32) {
            throw new Error('VC token serial number must be exactly 32 bytes in length')
          }
      ...
      if (outputsToAdmit.length === 0) {
        throw new Error('VC topic manager: no outputs admitted!')
      }
    ...
    if (previousCoins !== undefined && previousCoins.length > 0) {
      console.log(`Consumed ${previousCoins.length} previous VC ${previousCoins.length === 1 ? 'coin' : 'coins'}!`)
    }

    if (outputsToAdmit.length === 0 && (previousCoins === undefined || previousCoins.length === 0)) {
      console.warn('No VC outputs admitted, and no previous VC coins were consumed.')
    }
  ...
  async getMetaData(): Promise<{
    ...
    return {
      name: 'VC Topic Manager',
      shortDescription: 'VC Resolution Protocol'
    }
  }
}
